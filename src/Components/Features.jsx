    ...
    <section className="py-14 bg-gray-200">
      <div className="max-w-screen-xl mx-auto px-4 text-gray-600 md:px-8">
        <div className="max-w-xl space-y-3">
          ...
          <p className="text-gray-800 text-3xl font-bold sm:text-4xl"> Features of our Solution </p>
          ...
          <ul className="grid gap-y-8 gap-x-12 sm:grid-cols-2 lg:grid-cols-3">
            {iconArray.map((icon, idx) => (
              <li key={idx} className="space-y-3">
                <div className="w-16 h-16 border text-indigo-600 rounded-lg flex items-center justify-center bg-gray-300 transition duration-300 ease-in-out hover:bg-indigo-600 hover:text-white transform hover:scale-105" style={{ transform: `rotate(${rotations[idx]}deg)` }} onClick={() => handleRotate(idx)}>
